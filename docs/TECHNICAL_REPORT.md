...
3. **Use Advanced Chemistry for carbon accounting.** On the same system boundary, the simple mass-factor model runs ~5–6% *higher* than the clinker-chemistry model for high-clinker OPC (its fixed factor of 0.912 kg CO₂/kg cement vs the advanced cement term of ≈0.864 kg/kg: clinker calcination + kiln fuel 0.95·0.88 = 0.836, plus process electricity 0.11 kWh/kg × grid ≈ 0.0275 — added in R8.0, which closed most of what used to be an ~8–9% gap of silently dropped scope). The advanced tier also computes blended cements properly: LC3's cement term lands at ≈0.544 kg/kg (clinker half + its calcined-clay/limestone constituents + process electricity), inside the published 0.50–0.65 EPD band. *(An earlier revision claimed the simple model **underestimates** by 10–15%; that compared mismatched system boundaries — simple full-mix vs advanced cement-only — and is corrected here.)*
