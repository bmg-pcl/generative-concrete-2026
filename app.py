...
with tab2:
    ...
    st.info(
        "Generated by a transparent **GA-based inverse designer**: it searches — within the "
        "training-data envelope — for mixes whose model-predicted strength matches your target, and "
        "returns the spread of the best candidates. This is an honest, easy-to-read stand-in for a "
        "learned posterior (a trained BayesFlow flow is future work; see `docs/FIX_PLAN.md`).",
        icon="🧬",
    )
    st.markdown("""
    **How to use:** Enter a target compressive strength. The 3D surface shows the spread of mix
    designs that achieve it. Denser regions indicate parameter combinations the search favours.
    """)
